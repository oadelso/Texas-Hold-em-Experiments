#Console Input
#Expeiment 2
#One Agent will have a lower level of information than the other by only carrying out 5 simulations rather than 10
#HUTable and HUbettinground R functions are edited so that both agents use full kelly and carry out a total
#of 100 simulations to arrive at their expected probability of winning 
...
HUtable<-function(games=10,bigblind=1,initialw=100){
  ...
  for (i in 1:games){
    ...
    behavior=1;
    kellyfrac=0.5;
    nsimulations=5; #less informed agent
    if (sbindex==1){
